...
describe("Form creator page", () => {
  ...
  /**
   * Tests commented out because project has run out of time and we don't know how to fix a bug where cy.intercept isn't intercepting the get request
   */
  ...
  //       {
  //         method: 'GET',
  //         url: 'localhost/api/teams?canModifyForms=true'
  //       },
  //     ({teamId: 1, teamMembers: [({canManageTeam: true, canModifyForms: true, username: "Test TeamMember"} as TeamMember)], teamName: "Test TeamName"} as TeamView)
  //   );
  // });
  //
  //
  ...
  //   cy.get('button').contains("Multiple checkboxes").click();
  //
  //   cy.get('div[name="checkbox-group-container"]').get('button').contains("+ Add new checkbox").click();

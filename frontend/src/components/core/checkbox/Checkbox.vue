<template>
  <div class="checkbox__container">
    ...
    <EditableComponent edit-component-css="checkbox__label-edit" :value="selectionValue.label" @finish-editing="updateLabel" :mode="this.mode">
      <label class="checkbox__label" :for="id">{{ selectionValue.label }}</label>
    </EditableComponent>
    <button v-if="isDeletable && this.mode === 'Edit'" class="hidden-button ph-trash" @click="$emit('deleteCheckbox')"/>
  </div>
</template>
